						...
						if($created_event_id > 0){
							...
							if(is_array($roundOneData) && count($roundOneData) > 0){
								...
								foreach($roundOneData as $k =>$v){
									...
									Eventmatch::insertEventMatchToEvent($event_id, $created_event_id, $match_id, $round, $team1, $team2);
								}
							}
						}
					}
		...
		if(!empty($eventmatches) && count($eventmatches)>0){
			foreach ($eventmatches as $key => $em) {
				...
				switch($status){
					...
					Eventpool::insertWonTeamIntoPool($event_id, $created_event_id, $nextRound, $teamData);
