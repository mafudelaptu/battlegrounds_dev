	...
	if (!disabled) {
		...
		l(event_id);
				...
				if (now <= endTimestamp) {
					...
					$
					.ajax({
						url : ARENA_PATH+'/find_match/checkJoinQueue',
						type : "POST",
						dataType : 'json',
						data : {
						...
						success : function(result) {
							...
							switch(result.status){
								...
								$
								.ajax({
									url : ARENA_PATH+'/events/joinEvent',
									type : "POST",
									dataType : 'json',
									data : {
										event_id : event_id
...
function initNextEventClock(){
	var clock = $("#eventClock");
	// create a new javascript Date object based on the timestamp
	// multiplied by 1000 so that the argument is in milliseconds, not seconds
	timestamp = clock.attr("data-date");
...
function setEventReadyStatus(event_id, value){
	 $.ajax({
			url : ARENA_PATH+'/events/setEventReadyStatus',
			type : "POST",
			dataType : 'json',
			data : {
				event_id : event_id,
