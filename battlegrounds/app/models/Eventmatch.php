...
class Eventmatch extends Eloquent {
	...
	public static function insertEventMatchToEvent($event_id, $created_event_id, $match_id, $round, $team1, $team2){
		$insertArray = array(
			"event_id" => $event_id,
	...
	public static function getAllPlayedMatches(){
		...
		->select("eventmatches.event_id", "eventmatches.created_event_id", "eventmatches.team_won_id")
		->distinct()
		;
	}
	...
	public static function getEventMatchesStatus($event_id, $created_event_id){
		$ret = array();
		$data = Eventmatch::getEventmatches($event_id, $created_event_id)->get();
		if(!empty($data)){
			$retArray = array();
			foreach($data as $k => $v){
				$tmpArray = array(
