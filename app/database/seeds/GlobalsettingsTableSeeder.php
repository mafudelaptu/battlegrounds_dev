	...
	public function run()
	{
		...
		switch(App::environment()){
			case "dc":
				$ForumArenaActive = 1;
				$KickAFKUsers = 0;
				$SocketLink = "http://dotacinema.com:3000";
				$ForumLink = "http://www.dotacinema.com/forum/";
				$LoginViaKey = 1;
				$FakeLoginAllowed = 0;
				break;
			case "local_ih":
		...
		$globalsettings = array(
			...
			array("name" => "ForumArenaActive","value" => "", "active" => $ForumArenaActive, "created_at" => new DateTime, "updated_at" => new DateTime, "description" =>"set if arena should be accessible or not(setting via 'active' column)"),
			...
			array("name" => "PingLockTime","value" => 15, "active" => 1, "created_at" => new DateTime, "updated_at" => new DateTime, "description" =>"set lock time for ping notification, in which interval a user is pingable in seconds. (INTEGER, Default:15) "),
			array("name" => "KickAFKUsers","value" => "", "active" => $KickAFKUsers, "created_at" => new DateTime, "updated_at" => new DateTime, "description" =>"set lock time for ping notification, in which interval a user is pingable in seconds. (INTEGER, Default:15) "),
			array("name" => "ChatHistoryCount","value" => 50, "active" => 1, "created_at" => new DateTime, "updated_at" => new DateTime, "description" =>"set the number of older chat messages that has to be displayed on reload(INTEGER, Default:50) "),
			array("name" => "SteamGame","value" => "", "active" => $SteamGame, "created_at" => new DateTime, "updated_at" => new DateTime, "description" =>"set via active param if the game is a steam game(INTEGER, Default:1) "),
			array("name" => "SocketLink","value" => $SocketLink, "active" => 1, "created_at" => new DateTime, "updated_at" => new DateTime, "description" =>"set the link for socket connection(STRING, Default:localhost) "),
			array("name" => "ScreenshotUploadSourceDir","value" => "server/php/files/", "active" => 1, "created_at" => new DateTime, "updated_at" => new DateTime, "description" =>"set source directory where screenshot files gets uploaded - with '/' at end (STRING, Default:'server/php/files/')."),
			array("name" => "ScreenshotUploadDestDir","value" => "files/screenshots/", "active" => 1, "created_at" => new DateTime, "updated_at" => new DateTime, "description" =>"set destination directory where screenshot files gets moved after upload - with '/' at end (STRING, Default:'files/screenshots/')."),
			array("name" => "LoginViaKey","value" => "", "active" => $LoginViaKey, "created_at" => new DateTime, "updated_at" => new DateTime, "description" =>"set if users have to activate a Key before joining System (setting via 'active' column [enabled/disabled])."),
			array("name" => "FakeLoginAllowed","value" => "", "active" => $FakeLoginAllowed, "created_at" => new DateTime, "updated_at" => new DateTime, "description" =>"set if fake-Login is allowed throught url: '/fakelogin' and '/fakelogin/{user_id}' (setting via 'active' column [enabled/disabled])."),
