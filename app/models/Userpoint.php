...
class Userpoint extends Eloquent {
	...
	public static function insertPointChanges($match_id, $team_won_id, $matchdetails, $playerData, $matchmode_id, $matchtype_id){
		...
		$leaverData = Match::getLeaverTeamCounts($match_id, $matchdetails, $matchtype_id, $team_won_id);
				...
				foreach ($playerData[$i] as $key => $md) {
					...
					if(in_array($user_id, $leaverData['leaver'])){
						...
						if($team_won_id == $team_id){
							...
							//dd($leaverData['handicapped']);
							if($leaverData['handicapped'] == $team_id){
								$pointChange = -0;
								$pointsType = 2;
							}
							else{
	...
	public static function insertGeneralPointChange($user_id, $match_id, $event_id, $pointtype_id, $pointschange, $matchtype_id, $matchmode_id){
		$insertArray = array(
			"user_id" => $user_id,
			"match_id" => $match_id,
			"event_id" => $event_id,
	...
	public static function getPointsHistoryData($matchmode_id, $matchtype_id, $user_id, $count="*"){
		...
		$data->leftJoin("matches", "matches.id", "=", "userpoints.match_id")
	...
	public static function deleteAllUserPointsToMatch($match_id){
		Userpoint::where("match_id", $match_id)->delete();
	}
}