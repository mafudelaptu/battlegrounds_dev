...
class Eventregistration extends Eloquent {
	...
	public static function updateReadyOfUser($user_id, $event_id, $value){
		$updateArray = array(
			"ready"=>$value,
			"updated_at"=> new DateTime,
			);
		Eventregistration::getRegistrationOfUser($event_id, $user_id)->update($updateArray);
	}

	public static function getAcceptedCount($event_id){
		return Eventregistration::where("event_id", $event_id)
		->where("ready",1)
		->count()
		;
