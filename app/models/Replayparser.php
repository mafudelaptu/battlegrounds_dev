			...
			foreach($alleReplayFolders as $k => $v){
				...
				if(count($alleJsonFiles)>0){
					...
					foreach($alleJsonFiles as $datei){
						...
						$json_a = json_decode($string,true, 512,JSON_BIGINT_AS_STRING);
						//}
	...
	public static function checkAllPlayersIn($matchID, $playersArray){
		...
		if($matchID > 0){
			...
			if(is_array($playersArray) && count($playersArray) > 0){
				//array umschrieben
				$allPlayersNew = array();
				foreach ($allPlayersNew as $key => $value) {
					$allPlayersNew[] = $value->user_id;
				}
				
				foreach ($playersArray as $k => $v) {
					$steamID = $v['steamid'];
					if(is_array($allPlayersNew) && count($allPlayersNew) > 0){
						$key = "-1";
						foreach ($allPlayersNew as $kk => $vv) {
							if($steamID == $vv){
								$key = (int)$kk;
								break;
							}
						...
						if($key != "-1"){
							unset($allPlayersNew[(int)$key]);
						}

					}
				}
				if(count($allPlayersNew) === 0){
					$ret['status'] = true;
				}
				else{
					$ret['status'] = false;
