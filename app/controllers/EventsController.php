...
class EventsController extends BaseController {
	...
	public function index()
	{
		...
		$retER2 = array();
		$eventStatsRet = array();
		$eventWinnersRet = array();
		$mostEventWinsRet = array();
		$lastEvents = array();
		$allEventTypesRet = null;
		$isRdyForEvent = false;
		$nextEventState = array();
		...
		// add accepted count
			$nextEvent['acceptedCount'] = Eventregistration::getAcceptedCount($event_id);
...
//dd($retAllowed);
		...
		// 
			...
			$matchtype_id = 0;
			$created_event_id = 0;
			$retAllowed['status'] = false;
			$retER2['status'] = false;
			$eventStatsRet['data'] = null;
			$eventWinnersRet['data'] = null;
			$mostEventWinsRet['data'] = null;
			$lastEvents['data'] = null;
			$nextEventState['status'] = null;
		}
	...
	public function setEventReadyStatus(){
		$ret = array();
		if(Auth::check()){
			if (Request::ajax()){
				$user_id = Auth::user()->id;
				...
				Eventregistration::updateReadyOfUser($user_id, $event_id, $value);
				$ret['status'] = true;
			}
		}
		return $ret;
