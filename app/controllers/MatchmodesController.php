...
class MatchmodesController extends BaseController {
	...
	public function getQuickJoinModes(){
		...
		return $modes->select("matchmodes.created_at","matchmodes.name", "matchmodes.id as matchmode_id", "matchmodes.shortcut")->get();
	}
	...
	public function getMatchmodeData(){
		...
		if(Auth::check()){
			if (Request::ajax()){
				$allInputs = Input::all();
				$matchtype_id = reset($allInputs);
				$selectedArray = Input::get("selectedArray");
				$data = DB::table("matchmodes");
				if($matchtype_id == "2"){ // when 1vs1  there is just 1 mm
					$data = $data->where("id",GlobalSetting::get1vs1Matchmode())->select("matchmodes.created_at","matchmodes.name", "matchmodes.id as matchmode_id", "matchmodes.shortcut")->first();
					$ret =  array(0=>$data);
				}
				else{
					if(!empty($selectedArray) && count($selectedArray)>0){
						...
						$ret = $data->select("matchmodes.created_at","matchmodes.name", "matchmodes.id as matchmode_id", "matchmodes.shortcut")->get();
					}
					else{
						$ret = array();
					}
