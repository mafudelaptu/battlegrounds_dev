...
* Date: 2013-01-01
*/
class OneVsOneQueue extends Queue{

	const matchType = 8; // 1vs1QueueID in MatchType-DB
	/*
	 * Copyright 2013 Artur Leinweber
	* Date: 2013-01-01
	*/
			...
			if($this->userNotAlreadyInQueue($steamID)){
				...
				$Match = new Match();
					if(is_array($matchmodes) AND count($matchmodes)>0){
						...
						$retUP = $UserPoints->getGlobalPointsOfUser($steamID, OneVsOneQueue::matchType);
						$points = $retUP['data'];
						foreach($matchmodes as $k => $v){
							...
							if(is_array($regions) AND count($regions)>0){
								...
								foreach($regions as $kr => $vr){
									...
									$tmpArray['MatchTypeID'] = OneVsOneQueue::matchType;
	...
	*/
	function getPlayersInQueue(){
		...
		$sql = "SELECT DISTINCT SteamID
				FROM Queue
				WHERE MatchTypeID = ".(int) OneVsOneQueue::matchType."
						";
	...
	*/
	function getPlayersInMatchCount(){
		...
		$sql = "SELECT Count(DISTINCT m.MatchID)*2 as Count
	...
	*/
	function getMatchModeCounts($moreInfo = false){
		...
		$sql = "SELECT DISTINCT q.MatchModeID, SteamID, mm.Name as MatchModeName, mm.Shortcut as MatchModeSc
				...
				WHERE MatchTypeID = ".OneVsOneQueue::matchType."
